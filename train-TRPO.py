...
from sb3_contrib import TRPO  # TRPO는 sb3-contrib에서 제공

# 저장할 폴더 경로 설정
save_dir = "./trpo_multi_input_models/"  # 모델 저장 디렉토리
os.makedirs(save_dir, exist_ok=True)  # 디렉토리가 없으면 생성

# 모델 저장 경로 설정
trpo_model_path = os.path.join(save_dir, "trpo_latest.zip")  # 최신 TRPO 모델 저장 경로
resume_training = True  # True로 설정하면 이전 모델에서 학습 재개

# TRPO 모델 로드 또는 새로 생성
if resume_training and os.path.exists(trpo_model_path):
    print(f"Loading trained TRPO model from {trpo_model_path}")
    trpo_model = TRPO.load(trpo_model_path, env=env)
else:
    if resume_training:
        print(f"Warning: TRPO model not found at {trpo_model_path}. Starting training from scratch.")
    ...
    trpo_model = TRPO(
        ...
        learning_rate=1e-4,
        gamma=0.99,  # 할인 계수
        tensorboard_log="./trpo_tensorboard_multi_input/",  # 텐서보드 로그 경로
        #policy_kwargs={
        #    "max_kl": 0.01,         # 최대 KL 발산 값
        #    "cg_iters": 10,         # Conjugate Gradient 반복 횟수
        #    "cg_damping": 0.1,      # CG 감쇠
        #    "vf_iters": 5,          # 가치 함수 최적화 반복 횟수
        #    "vf_stepsize": 1e-3,    # 가치 함수 학습 속도
        #},
    )
...
# 학습 시작
trpo_model.learn(total_timesteps=1000000, callback=callback)

# 모델 저장
current_date = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
print(f"Start {current_date}_model saving at {save_dir}")
trpo_model_save_path = os.path.join(save_dir, f"trpo_{current_date}.zip")
trpo_model.save(trpo_model_save_path)
print(f"TRPO Model saved at {save_dir}")

# 최신 모델 업데이트
print(f"Start latest_model updating at {save_dir}")
trpo_model_latest_path = os.path.join(save_dir, "trpo_latest.zip")

# 최신 모델을 덮어쓰기 위해 기존 모델 파일 복사
if os.path.exists(trpo_model_latest_path):
    os.remove(trpo_model_latest_path)  # 기존 파일 삭제
shutil.copyfile(trpo_model_save_path, trpo_model_latest_path)