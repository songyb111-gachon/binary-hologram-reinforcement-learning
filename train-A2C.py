...
# 모델 저장 경로 설정
a2c_model_path = os.path.join(save_dir, "a2c_latest.zip")  # 최신 A2C 모델 저장 경로
resume_training = True  # True로 설정하면 이전 모델에서 학습 재개

# A2C 모델 로드 또는 새로 생성
if resume_training and os.path.exists(a2c_model_path):
    print(f"Loading trained A2C model from {a2c_model_path}")
    a2c_model = A2C.load(a2c_model_path, env=env)
else:
    if resume_training:
        print(f"Warning: A2C model not found at {a2c_model_path}. Starting training from scratch.")
    ...
    a2c_model = A2C(
        ...
        verbose=2,
        learning_rate=1e-4,
        n_steps=600,  # Advantage를 계산하기 위한 스텝 수
        gamma=0.99,  # 할인 계수
        ...
        tensorboard_log="./a2c_tensorboard_multi_input/",  # 텐서보드 로그 경로
    )
...
# 학습 시작
a2c_model.learn(total_timesteps=1000000, callback=callback)

# 모델 저장
current_date = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
print(f"Start {current_date}_model saving at {save_dir}")
a2c_model_save_path = os.path.join(save_dir, f"a2c_{current_date}.zip")
a2c_model.save(a2c_model_save_path)
print(f"A2C Model saved at {save_dir}")

# 최신 모델 업데이트
print(f"Start latest_model updating at {save_dir}")
a2c_model_latest_path = os.path.join(save_dir, "a2c_latest.zip")

# 최신 모델을 덮어쓰기 위해 기존 모델 파일 복사
if os.path.exists(a2c_model_latest_path):
    os.remove(a2c_model_latest_path)  # 기존 파일 삭제
shutil.copyfile(a2c_model_save_path, a2c_model_latest_path)