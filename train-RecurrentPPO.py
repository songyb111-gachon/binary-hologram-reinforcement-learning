...
from debug_env import BinaryHologramEnv
...
# 저장할 폴더 경로 설정
save_dir = "./recurrent_ppo_models/"  # 모델 저장 디렉토리
os.makedirs(save_dir, exist_ok=True)  # 디렉토리가 없으면 생성

# 모델 저장 경로 설정
recurrent_ppo_model_path = os.path.join(save_dir, "recurrent_ppo_latest.zip")  # 최신 Recurrent PPO 모델 저장 경로
resume_training = True  # True로 설정하면 이전 모델에서 학습 재개

# Recurrent 환경 생성 (기존 환경과 동일하게 사용)
env = BinaryHologramEnv(
    target_function=model,
    trainloader=train_loader,
)

# Recurrent PPO 모델 로드 또는 새로 생성
if resume_training and os.path.exists(recurrent_ppo_model_path):
    print(f"Loading trained Recurrent PPO model from {recurrent_ppo_model_path}")
    recurrent_ppo_model = RecurrentPPO.load(recurrent_ppo_model_path, env=env)
else:
    if resume_training:
        print(f"Warning: Recurrent PPO model not found at {recurrent_ppo_model_path}. Starting training from scratch.")
    print("Starting training from scratch.")
    recurrent_ppo_model = RecurrentPPO(
        "MultiInputLstmPolicy",  # LSTM 기반 정책 사용
        env,
        verbose=2,
        learning_rate=1e-4,
        n_steps=128,  # LSTM의 시간 단계 길이
        batch_size=32,  # 배치 크기 (n_steps의 배수로 설정해야 함)
        gamma=0.99,  # 할인 계수
        gae_lambda=0.95,  # GAE(Generalized Advantage Estimation)의 람다
        clip_range=0.2,  # 정책 손실 클리핑 범위
        ent_coef=0.01,  # 엔트로피 계수 (탐험 정도 제어)
        vf_coef=0.5,  # 가치 함수 손실 계수
        max_grad_norm=0.5,  # 그래디언트 클리핑
        tensorboard_log="./recurrent_ppo_tensorboard/",  # 텐서보드 로그 경로
    )

# 콜백 설정
max_episodes = 1000  # 원하는 에피소드 수
...
# 학습 시작
recurrent_ppo_model.learn(total_timesteps=1000000, callback=callback)

# 모델 저장
current_date = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
print(f"Start {current_date}_model saving at {save_dir}")
recurrent_ppo_model_save_path = os.path.join(save_dir, f"recurrent_ppo_{current_date}.zip")
recurrent_ppo_model.save(recurrent_ppo_model_save_path)
print(f"Recurrent PPO Model saved at {save_dir}")

# 최신 모델 업데이트
print(f"Start latest_model updating at {save_dir}")
recurrent_ppo_model_latest_path = os.path.join(save_dir, "recurrent_ppo_latest.zip")

# 최신 모델을 덮어쓰기 위해 기존 모델 파일 복사
if os.path.exists(recurrent_ppo_model_latest_path):
    os.remove(recurrent_ppo_model_latest_path)  # 기존 파일 삭제
shutil.copyfile(recurrent_ppo_model_save_path, recurrent_ppo_model_latest_path)