...
# 모델 저장 경로 설정
dqn_model_path = os.path.join(save_dir, "dqn_latest.zip")  # 최신 DQN 모델 저장 경로
...
if resume_training and os.path.exists(dqn_model_path):
    ...
    dqn_model = DQN(
        ...
        verbose=2,
        learning_rate=1e-4,
        buffer_size=50000,  # Replay buffer 크기
        learning_starts=50000,  # 학습 시작 단계
        batch_size=64,  # 배치 크기
        gamma=0.99,  # 할인 계수
        ...
        tensorboard_log="./dqn_tensorboard_multi_input/",  # 텐서보드 로그 경로
    )
...
# 학습 시작
dqn_model.learn(total_timesteps=1000000, callback=callback)

# 모델 저장
current_date = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
print(f"Start {current_date}_model saving at {save_dir}")
dqn_model_save_path = os.path.join(save_dir, f"dqn_{current_date}.zip")
dqn_model.save(dqn_model_save_path)
print(f"DQN Model saved at {save_dir}")

# 최신 모델 업데이트
print(f"Start latest_model updating at {save_dir}")
dqn_model_latest_path = os.path.join(save_dir, "dqn_latest.zip")
