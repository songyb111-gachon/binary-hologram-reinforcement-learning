...
# 저장할 폴더 경로 설정
save_dir = "./dqn_multi_input_models/"  # 모델 저장 디렉토리
os.makedirs(save_dir, exist_ok=True)  # 디렉토리가 없으면 생성

from sb3_contrib import QRDQN  # QRDQN은 sb3-contrib에서 제공됨

# 저장할 폴더 경로 설정
save_dir = "./qrdqn_multi_input_models/"  # 모델 저장 디렉토리
os.makedirs(save_dir, exist_ok=True)  # 디렉토리가 없으면 생성

# 모델 저장 경로 설정
qrdqn_model_path = os.path.join(save_dir, "qrdqn_latest.zip")  # 최신 QR-DQN 모델 저장 경로
resume_training = True  # True로 설정하면 이전 모델에서 학습 재개

# QR-DQN 모델 로드 또는 새로 생성
if resume_training and os.path.exists(qrdqn_model_path):
    print(f"Loading trained QR-DQN model from {qrdqn_model_path}")
    qrdqn_model = QRDQN.load(qrdqn_model_path, env=env)
else:
    if resume_training:
        print(f"Warning: QR-DQN model not found at {qrdqn_model_path}. Starting training from scratch.")
    print("Starting training from scratch.")
    qrdqn_model = QRDQN(
        "MultiInputPolicy",  # 멀티인풋 정책 사용
        env,
        verbose=2,
        learning_rate=1e-4,
        buffer_size=100000,  # Replay buffer 크기
        learning_starts=1000,  # 학습 시작 단계
        batch_size=64,  # 배치 크기
        gamma=0.99,  # 할인 계수
        train_freq=4,  # 학습 빈도 (매 4 스텝마다 학습)
        target_update_interval=10000,  # 타겟 네트워크 업데이트 주기
        exploration_fraction=0.1,  # 탐험 비율
        exploration_final_eps=0.01,  # 최종 탐험 확률
        tensorboard_log="./qrdqn_tensorboard_multi_input/",  # 텐서보드 로그 경로
        policy_kwargs={"n_quantiles": 25},  # QR-DQN의 quantiles 수
    )
...
# 학습 시작
qrdqn_model.learn(total_timesteps=1000000, callback=callback)

# 모델 저장
current_date = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
print(f"Start {current_date}_model saving at {save_dir}")
qrdqn_model_save_path = os.path.join(save_dir, f"qrdqn_{current_date}.zip")
qrdqn_model.save(qrdqn_model_save_path)
print(f"QR-DQN Model saved at {save_dir}")

# 최신 모델 업데이트
print(f"Start latest_model updating at {save_dir}")
qrdqn_model_latest_path = os.path.join(save_dir, "qrdqn_latest.zip")

# 최신 모델을 덮어쓰기 위해 기존 모델 파일 복사
if os.path.exists(qrdqn_model_latest_path):
    os.remove(qrdqn_model_latest_path)  # 기존 파일 삭제
shutil.copyfile(qrdqn_model_save_path, qrdqn_model_latest_path)