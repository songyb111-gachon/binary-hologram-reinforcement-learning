...
class BinaryHologramEnv(gym.Env):
    ...
    def step(self, action, z=2e-3):
        ...
        print(f"Time taken for 스텝 사이와 사이 (action choose time?) : {step_t:.6f} seconds")
        ...
        print(f"Time taken for simulate : {sim_t:.6f} seconds")
        ...
        print(f"Time taken for NumPy : {numpy_t:.6f} seconds")

        obs_time = time.time()
        obs = {"state_record": state_record, "state": state, "pre_model": pre_model, "recon_image": result_np, "target_image": target_image_np}
        obs_t = time.time() - obs_time
        print(f"Time taken for obs : {obs_t:.6f} seconds")
        ...
        print(f"Time taken for reward : {reward_t:.6f} seconds")
        ...
        if psnr_change < 0:
            ...
            print(f"Time taken for rollback : {rollback_t:.6f} seconds")
        ...
        print(f"Time taken for print : {print_t:.6f} seconds")
        ...
        print(f"Time taken for terminated : {terminated_t:.6f} seconds")
        ...
        print(f"Time taken for max_steps : {max_steps_t:.6f} seconds")
        # 관찰값 업데이트
        #info = {
        #    "psnr_before": psnr_before,
        #    "psnr_after": psnr_after,
